...
Convex::Convex(const Convex &convex) {
...
/*
 */
Convex &Convex::operator=(const Convex &convex) {
	
	if(this == &convex) return *this;
...
/*
 */
int Convex::create(const vec3 *v,int num_vertex,float error) {
	...
	return create(vertex.get(),vertex.size(),error);
}

/*
 */
int Convex::create(const dvec3 *v,int num_vertex,float error) {
	
	clear();
	
	if(num_vertex < 4) return 0;
	
	// copy vertices
	vertex.resize(num_vertex);
	Math::memcpy(vertex.get(),v,sizeof(dvec3) * num_vertex);
	
	// optimize vertices
	int num_optimized_vertex = 0;
	quickSort2(vertex.get(),vertex.size(),vertex_compare);
	for(int i = 0; i < vertex.size(); i++) {
		if(i == 0 || vertex_compare(vertex[num_optimized_vertex - 1],vertex[i])) vertex[num_optimized_vertex++] = vertex[i];
	}
	...
	// convex threshold
	dvec3 min = dvec3_infinity;
	dvec3 max = -dvec3_infinity;
	for(int i = 0; i < vertex.size(); i++) {
		min = ::min(min,vertex[i]);
		max = ::max(max,vertex[i]);
	}
	threshold = ::max(length(max - min) / 2.0 * saturate(error),0.0) + EPSILON;
	...
	for(int i = 0; i < vertex.size(); i++) {
		if(vertex[v0][axis] < vertex[i][axis]) v0 = i;
		if(vertex[v1][axis] > vertex[i][axis]) v1 = i;
	}
...
/*
 */
dvec3 Convex::getClosestPoint(const dvec3 &point) const {
	...
	for(int i = 0; i < faces.size(); i++) {
		...
		if(distance < Math::abs(d)) continue;
		int inside = 1;
...
/*
 */
void Convex::getBoundBox(dvec3 &min,dvec3 &max) const {
...
/*
 */
int Convex::vertex_compare(const dvec3 &v0,const dvec3 &v1) {
...
int Convex::triangle_compare(const Triangle *t0,const Triangle *t1) {
...
/*
 */
void Convex::add_vertex(Triangle *t,int v) {
...
Convex::Triangle *Convex::add_triangle(int v0,int v1,int v2) {
...
/*
 */
int Convex::create_edges(const Vector<Triangle*> &triangles) {
...
/*
 */
int Convex::create_convex() {
	
	VectorStack<int> removed_vertex;
	VectorStack<int> counter(vertex.size());
	VectorStack<Triangle*> removed_triangles;
	...
	while(iterations-- > 0) {
		...
		if(triangles.size() > 2 && distance < threshold) break;
...
/*
 */
int Convex::create_vertex() {
	
	VectorStack<int> vbuf(vertex.size());
	VectorStack<int> counter(vertex.size());
	VectorStack<dvec3> old_vertex = vertex;
	vertex.clear();
...
/*
 */
int Convex::create_faces() {
	
	VectorStack<int> indices;
	VectorStack<Triangle*> face_triangles;
	...
	for(int i = 0; i < triangles.size(); i++) {
		...
		for(; i < triangles.size() - 1; i++) {
			if(triangle_compare(face_triangles[0],triangles[i + 1])) break;
		...
		for(int j = 0; j < edges.size(); j++) {
			if(edges[j].counter != 1) edges.removeFast(j--);
		}
		...
		for(int j = 0; j < edges.size(); j++) {
			...
			for(int k = 0; k < edges.size(); k++) {
				if(dot(clip,vertex[edges[k].v[0]] - v0) > EPSILON) {
					convex = 0;
					break;
				}
			}
		...
		while(1) {
			...
			for(int j = 0; j < edges.size(); j++) {
				...
				if(indices.size() == 0) {
					...
					edges.removeFast(j--);
				} else if(indices[indices.size() - 1] == e->v[0]) {
					indices.append(e->v[1]);
					edges.removeFast(j--);
				}
			}
			if(num_indices == indices.size()) break;
		}
